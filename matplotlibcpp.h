...
namespace matplotlibcpp
{
    ...
    template <typename Numeric>
    PyObject *plot(const std::vector<Numeric> &x, const std::vector<Numeric> &y, const std::map<std::string, std::string> &keywords)
    {
...
// a non-numpy alternative for `detail::get_2darray()`.
    ...
    PyObject *create_figure()
    {
        ...
        PyObject *fig_args = PyTuple_New(0); // No positional arguments
        PyObject *fig_kwargs = PyDict_New(); // No keyword arguments

        PyObject *fig = PyObject_Call(detail::_interpreter::get().s_python_function_figure, fig_args, fig_kwargs);
        Py_DECREF(fig_args);
    ...
    void add_subplot(PyObject *fig, int nrows, int ncols, int index)
    {
        if (!fig)
        {
            throw std::runtime_error("Invalid figure object.");
        }

        // Prepare the arguments for the add_subplot function
        PyObject *args = PyTuple_New(3);
        ...
        PyObject *res = PyObject_CallMethod(fig, "add_subplot", "OOO", PyTuple_GetItem(args, 0), PyTuple_GetItem(args, 1), PyTuple_GetItem(args, 2));
        Py_DECREF(args);

        if (!res)
        {
            throw std::runtime_error("Call to add_subplot() failed.");
        }
    ...
    template <typename NumericX, typename NumericY>
    PyObject *semilogy(const std::vector<NumericX> &x, const std::vector<NumericY> &y, const std::string &s = "")
    {
        assert(x.size() == y.size());
    ...
    inline void figure_size(PyObject *fig, double w, double h)
    {
        if (!fig)
        {
            throw std::runtime_error("Invalid figure object.");
        ...
        PyObject *res = PyObject_CallMethod(fig, "set_size_inches", "dd", w / 100.0, h / 100.0);
    ...
    void set_all_subplots_shared_x(PyObject *fig)
    {
        ...
        PyObject *axes_list = PyObject_CallMethod(fig, "get_axes", nullptr);
        if (!axes_list)
        {
            throw std::runtime_error("Call to get_axes() failed.");
        }

        Py_ssize_t num_axes = PyList_Size(axes_list);
        if (num_axes < 1)
        {
            Py_DECREF(axes_list);
            return;
        ...
        PyObject *first_ax = PyList_GetItem(axes_list, 0); // Borrowed reference
        for (Py_ssize_t i = 1; i < num_axes; ++i)
        {
            PyObject *ax = PyList_GetItem(axes_list, i); // Borrowed reference
            PyObject *res = PyObject_CallMethod(ax, "sharex", "O", first_ax);
            if (!res)
            {
                Py_DECREF(axes_list);
                throw std::runtime_error("Call to sharex() failed.");
            }
            Py_DECREF(res);
        }
    ...
    PyObject *get_first_line(PyObject *ax)
    {
        if (!ax)
        {
            throw std::runtime_error("Invalid axes object.");
        }

        PyObject *lines = PyObject_CallMethod(ax, "get_lines", nullptr);
        if (!lines)
        {
            throw std::runtime_error("Call to get_lines() failed.");
        }

        PyObject *first_line = PyList_GetItem(lines, 0); // Borrowed reference
        Py_INCREF(first_line);                           // Increment the reference count since we'll return this object
        Py_DECREF(lines);                                // Decrement the reference count of the list

        return first_line;
    }

    PyObject *get_line_number(PyObject *ax, const int nb = 0) // starts at 0
    {
        if (!ax)
        {
            throw std::runtime_error("Invalid axes object.");
        ...
        PyObject *first_line = PyList_GetItem(lines, nb); // Borrowed reference
        Py_INCREF(first_line);                            // Increment the reference count since we'll return this object
        Py_DECREF(lines);                                 // Decrement the reference count of the list
    ...
    void set_label(const std::string &label, const int line_number = 0)
    {

        PyObject *ax = gca();
        PyObject *line = get_line_number(ax, line_number);

        if (!line)
        {
            throw std::runtime_error("Invalid line object.");
        }

        detail::_interpreter::get();

        PyObject *pylabel = PyUnicode_FromString(label.c_str());
        PyObject *res = PyObject_CallMethod(line, "set_label", "O", pylabel);
        Py_DECREF(pylabel);

        if (!res)
        {
            throw std::runtime_error("Call to set_label() failed.");
        }
    ...
    inline void subplots(long nrows, long ncols, long plot_number, bool sharex = false, bool sharey = false)
    {
        detail::_interpreter::get();
        ...
        // construct keyword args
        PyObject *kwargs = PyDict_New();
        PyDict_SetItemString(kwargs, "sharex", PyBool_FromLong(sharex ? 1 : 0));
        PyDict_SetItemString(kwargs, "sharey", PyBool_FromLong(sharey ? 1 : 0));
    ...
    void set_yscale_log()
    {
        ...
        PyObject *res = PyObject_CallMethod(ax, "set_yscale", "s", "log");
        if (!res)
        {
            PyErr_Print(); // Print the error to get more information about what went wrong
            throw std::runtime_error("Call to set_yscale_log() failed.");
        }
    ...
    inline PyObject *plot(const std::vector<double> &x, const std::vector<double> &y, const std::map<std::string, std::string> &keywords)
    {
